...
Diese Stunde beschäftigten wir uns mit **GitHub**. Da wir noch Anfänger sind, wollen wir wissen, wie man den Stundenblog auf GitHub erstellen kann. Wir schauten uns [GitHub Help](https://help.github.com/articles/basic-writing-and-formatting-syntax/#quoting-code) an, um die grundlegende Schreibsyntax zu lernen. Dadurch haben wir erfahren, wie man Screenshots oder Links einfügen kann. Zudem informierten wir uns darüber, was *Branching* und *Commits* sind. 
...
Wir wollten einen Hund mit Hilfe von Tasten horizontal bewegen können. Die Sprites sind grundsätzlich normale Pfeile und haben kein besonderes Aussehen. Um diese Sprites anschaulicher zu gestalten, kann man dafür entweder bei der Menüleiste bereits vorgegebene Figuren unter *Costumes* einfügen oder bereits heruntergeladene unter *Import*.

![informatik costumes](https://user-images.githubusercontent.com/42734752/48846290-07b9cc00-ed9f-11e8-92a2-39221e3ed29b.png)

Wir haben bei diesem Versuch eine Hundgrafik aus dem Internet importiert. Dafür ist es wichtig, dass es sich um ein PNG handelt, also transparent ist. Um das Sprite bewegen zu können, haben wir auf dem Script den Befehl, unter *Control* Taste 0, *move 10 steps (Motion)* eingefügt. Damit der Hund nicht von der Bildschirmansicht verschwinden kann, haben wir den Befehl *if on edge bounce* eingefügt, so dass er sich immer von links nach rechts und zurück bewegt.

![1](https://user-images.githubusercontent.com/42734752/44729547-5e38a680-aadf-11e8-9c5f-0ea227e6f0d4.jpg)

 Zusätzlich sollte der Hund "Hello" für zwei Sekunden sagen, wenn man eine spezifische Taste betätigt. Dafür haben wir, unter *Control: Leertaste*, den Befehl von *Looks* *say hello for 2 seconds* hinzugefügt.
...
# 10.09.18
  ...
  Nachdem wir uns für die blockbasierte Programmiersprache **Snap** entschieden haben, fingen wir den verfügbaren Kurs [*Beauty and Joy of computing*](https://bjc.edc.org/bjc-r/course/bjc4nyc.html) an, der von den Universitäten California, Berkeley und des Education Development Centers ausgearbeitet wurde. Der Kurs umfasst sieben Units, die unterschiedlichen Niveaus entsprechen. Als Anfänger wollen wir zuerst eine theoretische Grundlage erlangen.
...
Wir haben noch weiter Sachen ausprobiert, doch wir waren durch diesen Vorfall frustriert und beschlossen die nächsten Stunden etwas ganz neues auszuprobieren.

# 17.09.18

  Diesmal haben wir ein simples Spiel programmiert. Wir haben ein *Costume (Costume 1/Pfeil)*, welches man mit Hilfe der Pfeile der Tastatur bewegen kann, eingefügt. Dafür haben wir für die jeweiligen Pfeiltasten den Befehl *point in direction [...]* eingestellt, damit sich der Pfeil in die Richtung dreht, in die er sich bewegen soll. In das freie Feld konnte man die passende Gradzahl schreiben.
...
![apfelspiel](https://user-images.githubusercontent.com/42734752/45690620-15609480-bb57-11e8-9381-96494b7a5ce3.jpg)
  ...
  Dazu haben wir ein zweites *Costume (Costume 2/Apfel)* heruntergeladen, welches sich zufällig auf dem Bildschirm bewegt und jedesmal bevor es wieder auftaucht, verschwindet und dann an seiner neuen Position sichtbar wird. Dafür haben wir die Befehle *go to random position* eingefügt. Dann sollte er eine Sekunde warten (Befehl:*wait 1 sec*), bis er dann wieder durch den Befehl *show* sichtbar wird. Er ist dann fünf Sekunden lang sichtbar (Befehl: *wait 5 sec*) ist, damit man den Apfel mit dem ersten Sprite fangen kann. Das ganze sollte sich ständig (unendlich) wiederholen, weshalb wir dieses *Script* mit dem Befehl *forever* umschlossen.

  ![apfelspiel2](https://user-images.githubusercontent.com/42734752/45690638-214c5680-bb57-11e8-8203-81d8caf1d415.jpg)

Lenkt man *Costume 1* zu *Costume 2* und berührt dieses, wird *Costume 1* größer. Um dies zu erreichen, haben wir bei dem Block des ersten Sprites unter *Control* den Befehl *when touching Sprite 2* + *change size by 1* eingefügt. *Sprite 1* vergrößert sich also immer wenn es *Sprite 2* berührt um 1.
